...
(* =====================================================================
   ...
   ---------------------
   It computes and plots the background evolution of a flat
   Friedmann-Lemaitre-Robertson-Walker universe whose components have a
   constant equation of state. Everything is built from the Friedmann
   equation written as E(z) = H(z)/H0, so the only inputs are the density
   parameters today.

   It compares a LCDM model (Om=0.31, Or=9e-5, h=0.7) with three
   limiting universes: Einstein-de Sitter (only matter), open (matter plus
   curvature) and de Sitter (only a cosmological constant), and it studies
   the fiducial model in more detail: the densities of each component, the
   energy fractions, the eras and their transitions, the deceleration
   parameter and the cosmological distances.

   Structure of the file
   ---------------------
   Block 1  : constants and units.
   Block 2  : energy content of the fiducial LCDM model.
   Block 3  : basic functions E(z), E(a) and q(z).
   Block 3b : common format of the figures, manual legends and axis ticks.
   Block 4  : key numbers of the fiducial model, printed as a check.
   Block 5  : Figure 1, E(z) for the four universes.
   Block 6  : Figure 2, densities and eras.
   Block 7  : Figure 3, energy fractions.
   Block 8  : Figure 4, scale factor a(t).
   Block 9  : Figure 5, deceleration parameter q(z).
   Block 10 : Figure 6, cosmological distances.
   Block 11 : export of the figures to PDF.

   Output
   ------
   Five numbers printed in the notebook (the two equalities, the redshift at
   which the expansion starts to accelerate, q0 and the age of the universe)
   and six figures exported to vector PDF in the folder of the notebook.

   Two notes on the format
   -----------------------
   The uniform format of every figure (frame, grid, dashed markers) is set
   once with SetOptions in Block 3b, and the labels use real symbols (rho,
   Omega, Lambda, subscripts and superscripts) on the axes and legends.
   The legends are built by hand (Block 3b, function filaLey) because
   LineLegend draws the colour swatch as a glyph from an internal font
   that on some installations comes out broken (the strange symbols in
   front of each label). Drawing the swatch as a real line (Graphics +
   Line) makes the problem disappear, and putting everything in a single
   Row means the legend never splits into two lines.
   ===================================================================== *)
...
omL0 = 1 - omM0 - omR0 - omK0;   (* Lambda from closure (about 0.69). *)
...
(* ==== Block 3b: common format and labels with symbols (it must be evaluated). ==== *)
...
(* Four colours for the four distances (D_M blue, D_H purple, D_A orange, D_L green). *)
...
(*  Dash patterns (solid, dashed, dotted, dash-dot) to distinguish
   the curves also in black and white or for colour-blind readers. dash3 for
...
(* Short legend with the four distances (same order as the Plot of fig.6). *)
...
(* muestra accepts a dash pattern (dash) besides the colour, so the
   legend shows the same line pattern as the curve (distinguishable in black
   and white). *)
...
(* -- Variant for logarithmic axes with few decades: label 10^k
      at every decade and add short ticks without label at 2,3,...,9 of
...
(* The two equality vertical lines are in dash-dot style
   (verticalEq, as in plots_CPL.wl) and with a label giving the value of a.
   Positions (a axis):
...
(* Equality vertical lines in verticalEq plus a label with the value of z.
   Positions (1+z axis):
...
fig4 = Legended[
  ParametricPlot[{
    ...
    PlotPoints -> 120, MaxRecursion -> 4,   (* Fine sampling: it draws the whole almost vertical stretch of the Big Bang until it touches a=0 
...
(* Dash-dot vertical line (verticalEq) at z_acc (where LCDM crosses q=0,
   that is, the onset of acceleration) plus a label with that z. The other three
...
(* D_H = c/H(z) = dH0/E(z) (the Hubble distance, the radial BAO observable) is
   not an integral like the other three; it is local (D_H = dD_C/dz) and
   decreases with z, because H grows with z. Order and colours of fig.6: D_M
   blue, D_H purple, D_A orange, D_L green. Recall that in a flat universe
   D_A = D_M/(1+z) and D_L = (1+z) D_M, so D_A and D_L come from D_M.           *)
(* The y axis is on a logarithmic scale (LogPlot instead of Plot).
   The reason is that at z=3 the four distances are D_M=6293, D_H=945, D_A=1573
   and D_L=25172 Mpc. With a linear y axis, the automatic PlotRange discarded the
   ...
   the height, that is, the three small distances were squashed against the axis.
   In log scale the data range (42 to 25172 Mpc) is only about 2.8 decades:
...
(* =====================================================================
   Notes:
   - The format of all six figures is controlled in the SetOptions of Block 3b.
   ...
   - Greek letters: "\[Rho]"=rho, "\[CapitalOmega]"=Omega, "\[CapitalLambda]"=
     ...
     Mathematica turns them into the symbol (one sees a real rho, Omega...).
   - To vary the energy content, change omM0/omL0/omK0 (Block 2).
   - The equality vertical lines (fig.2, fig.3) and the q=0 one (fig.5)
     are in the dash-dot style 'verticalEq' with their value on the axis (a in
     fig.2, z in fig.3 and fig.5). Those of fig.2 and fig.3 are the same
     intersections, one on the a axis and the other on the 1+z axis
     (a = 1/(1+z)).
   - Fig.6 has four distances, including D_H = c/H(z) (Hubble distance, the
     radial BAO one). D_H decreases with z (it starts at c/H0 ~ 4283 Mpc) while
     the others grow; the maximum of D_A falls exactly where D_H = D_A (~z=1.6).
   - Fig.6 uses a logarithmic y axis (LogPlot). D_L grows up to
     25172 Mpc at z=3 while D_A stays at about 1700: on a linear scale either D_L
     was cut off or the other three were squashed. If the linear scale were
     still preferred, it is enough to change LogPlot to Plot, remove the
     FrameTicks and the GridLines of that figure and set
     PlotRange -> {{0, 3}, {0, 26000}} (but then D_H, D_M and D_A end up
     squeezed against the axis).
   ===================================================================== *)
...
(* 2. We export the 6 figures to high-quality vector PDF format. *)
